			   ...
			   preg_match('/^[a-zA-Z0-9]+$/', $_POST["password"])){
				...
				if($intentos < $maximoIntentos){
					...
					if($respuesta["nombreAdmin"] == $_POST["nombre"] && $respuesta["password"] == $_POST["password"]){
                        ...
                        $datosController = array("nombreCliente"=>$_POST["nombre"],
                            "passwordCliente"=>$_POST["password"]);

                        $respuesta = IngresoModels::ingresoModelCliente($datosController, "clientes");

                        $nombreAdminActual = $_POST["nombre"];


                        if($respuesta["nombreCliente"] == $_POST["nombre"] && $respuesta["passwordCliente"] == $_POST["password"]){

                            $_SESSION["validar"] = true;
                            $_SESSION["nombreCliente"] = $respuesta["nombreCliente"];
                            $_SESSION["rol"] = $respuesta["rol"];
                            $_SESSION["idCliente"] = $respuesta["idCliente"];

                            echo '<center><div class="alert alert-success"> Bienvenido <strong>'. ' '.	$_SESSION["nombreCliente"].'</strong></div>';
                            echo '<i class="fa fa-spinner fa-spin fa-3x fa-fw"></i>
								<span class="sr-only">Loading...</span></center>';
